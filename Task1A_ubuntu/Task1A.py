...
x1_dot = sp.symbol('x1_dot')
x2_dot = sp.symbol('x2_dot')
##################################################
...
def find_A_B_matrices( eq_points):
    ...
    ###### WRITE YOUR CODE HERE ################
    for t in enumerate(eq_points,1):
    	i,point=t
    	eq1_sub_point=A_matrix.subs({x1: point[x1],x2: point[x2]})
    	eq2_sub_point=B_matrix.subs({x1: point[x1],x2: point[x2]})
    	A_matrices.append(eq1_sub_point)
    	B_matrices.append(eq2_sub_point)
...
def find_eigen_values(A_matrices):
    ...
    for a in A_matrices:
	    ...
	    if (eig.as_real_img()[0]>=0): 
#eig.as_real_img() - it converts real+imaginary(3+2i) to (real,imaginary) 
	    	stable=False
	    	break
	if (stable):
