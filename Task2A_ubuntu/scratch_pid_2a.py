...
def sysCall_sensing():
    ...
    if message == sim.message_keypress:
        if data[0] == 2007:  # Increase translation set point
            setPt_trans += 0.002  # Smaller increment for translation
        elif data[0] == 2008:  # Decrease translation set point
            setPt_trans -= 0.002  # Smaller decrement for translation
        elif data[0] == 2009:  # Rotate left
            rotation_velocity = -5.0  # Smaller value for smooth rotation
        elif data[0] == 2010:  # Rotate right
            rotation_velocity = 5.0  # Positive value for right rotation
        elif data[0] == 113:  # 'q' key to decrease Prismatic_joint velocity
            sim.setJointTargetVelocity(Prismatic_joint, -0.05)  # Reduced velocity
        elif data[0] == 101:  # 'e' key to increase Prismatic_joint velocity
            sim.setJointTargetVelocity(Prismatic_joint, 0.05)  # Reduced velocity
        elif data[0] == 119:  # 'w' key to move arm joint forward
            sim.setJointTargetVelocity(arm_joint, 1)  # Reduced arm speed
        elif data[0] == 115:  # 's' key to move arm joint backward
            sim.setJointTargetVelocity(arm_joint, -1)  # Reduced arm speed
    else:
        # Stop the robot motors and joints if no key input
        sim.setJointTargetVelocity(Prismatic_joint, 0)
        sim.setJointTargetVelocity(arm_joint, 0)
        sim.setJointTargetVelocity(left_motor, 0)  # Stop left motor
        sim.setJointTargetVelocity(right_motor, 0)  # Stop right motor
